...
use crate::cli::{Cli, Command, OutputFormat, TimeRange, SessionInfoArgs};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::SessionInfo(args) => {
            let session = args.session.as_deref().ok_or(CliError::SessionRequired)?;
            run_session_info(session).await?
        }
    }
